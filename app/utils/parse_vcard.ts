...
export function parse(vcardString: string): VCard {
    const vcardLines = vcardString.trim().split(/\r?\n/);
    const vcard: VCard = {}; // need to make this into an array of vcards
    let lastKey: string | undefined;

    console.log("vcardLines", vcardLines)

    for (const line of vcardLines) {
        if (line.startsWith(' ')) {
            // continuation of the previous line
            if (lastKey) {
                vcard[lastKey] += line.slice(1);
            }
        } else {
